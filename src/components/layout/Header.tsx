...
import {
  ...
  Phone, LogIn, LogOut, LayoutDashboard,
} from "lucide-react";
...
function NavDropdown({
  ...
  const handleEnter = useCallback(() => { clearTimer(); setOpen(true); }, [clearTimer]);
  ...
  return (
      <div className="relative" onMouseEnter={handleEnter} onMouseLeave={handleLeave}>
        <button className={cn(
            ...
            active ? "text-primary" : "text-foreground/75 hover:text-foreground hover:bg-secondary/60"
        )}>
          {label}
        ...
        {open && (
            ...
            >
              <div className="w-80 bg-card border border-border rounded-2xl shadow-2xl overflow-hidden animate-fade-in">
                    ...
                    return (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="font-semibold text-sm text-card-foreground leading-tight">{item.title}</div>
                            <div className="text-xs text-muted-foreground mt-0.5">{item.desc}</div>
                          </div>
                          {item.badge && (
                              <span className={cn("text-xs font-bold px-2 py-0.5 rounded-full flex-shrink-0", item.badgeColor)}>
                        {item.badge}
                      </span>
                          )}
                        </button>
                ...
                {footer && <div className="border-t border-border bg-secondary/30 p-3">{footer}</div>}
              </div>
            </div>
        )}
      </div>
...
export function Header() {
  ...
  const navLinkClass = (href: string) => cn(
      ...
      isActive(href) ? "text-primary" : "text-foreground/75 hover:text-foreground hover:bg-secondary/60"
  );
          ...
          <div className="flex h-20 items-center justify-between gap-6">
            ...
            <div className="hidden lg:flex lg:items-center lg:gap-1">
              <NavDropdown
                  ...
                  footer={
                    <Link to="/orientation" className="flex items-center justify-between text-xs font-medium text-primary hover:underline">
              ...
              <Link to="/orientation" className={navLinkClass("/orientation")}>Orientation</Link>
              <Link to="/alumni" className={navLinkClass("/alumni")}>Alumni</Link>
              <Link to="/a-propos" className={navLinkClass("/a-propos")}>À propos</Link>
            ...
            <div className="hidden lg:flex lg:items-center lg:gap-3 flex-shrink-0">
              ...
              {isAuthenticated ? (
                  /* Utilisateur connecté — menu admin */
                  <div className="flex items-center gap-2">
                    <Button asChild variant="outline" size="sm" className="text-sm font-medium gap-1.5">
                      <Link to="/admin">
                  ...
                  /* Visiteur — boutons Connexion + CTA sessions */
                  <>
                    {/*<Button asChild variant="ghost" size="sm" className="text-sm font-medium gap-1.5 text-muted-foreground">*/}
                    {/*  <Link to="/admin/login">*/}
                    {/*    <LogIn className="h-4 w-4" />*/}
                    {/*    Connexion*/}
                    {/*  </Link>*/}
                    {/*</Button>*/}
                    <Button asChild size="sm" className="font-bold bg-primary hover:bg-primary/90 text-white px-5">
                      <Link to="/admin/login">
                        <LogIn className="h-4 w-4" />
                        Connexion
                      </Link>
          ...
          {mobileOpen && (
              <div className="lg:hidden border-t border-border py-4 animate-fade-in">
                <div className="flex flex-col gap-1">
                  ...
                  {/* Bootcamps */}
                  <button
                      onClick={() => setMobileExpanded(mobileExpanded === "bootcamps" ? null : "bootcamps")}
                ...
                <span className="flex items-center gap-2">
                  ...
                  Bootcamps
                    ...
                    <ChevronDown className={cn("h-4 w-4 transition-transform", mobileExpanded === "bootcamps" ? "rotate-180" : "")} />
                  </button>
                  {mobileExpanded === "bootcamps" && (
                      <div className="ml-4 flex flex-col gap-1 mb-2">
                        ...
                        <Link to="/orientation" onClick={() => setMobileOpen(false)} className="flex items-center gap-2 px-3 py-2 text-sm text-primary font-medium">
                    ...
                    { label: "Contact", href: "/contact", icon: Phone },
                      ...
                      <Link
                          ...
                          className={cn(
                              ...
                              isActive(item.href) ? "text-primary bg-primary/8" : "text-foreground/75 hover:text-foreground hover:bg-secondary/50"
                          )}
                      >
