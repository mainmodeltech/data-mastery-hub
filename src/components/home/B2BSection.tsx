            ...
            <div className="container mx-auto px-4 lg:px-8 relative z-10">
                ...
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 mb-16">
                    <div className="max-w-2xl">
                        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-accent/10 border border-accent/25 text-accent text-sm font-medium mb-4">
                ...
                <div className="grid lg:grid-cols-3 gap-6">
                    {B2B_OFFERS.map((offer, index) => {
                        ...
                        return (
                            <div
                                key={offer.id}
                                className={`relative rounded-2xl border-2 ${isAccent ? "border-accent/20" : "border-primary/20"} bg-card p-8 hover:shadow-lg transition-all duration-300 opacity-0 animate-fade-in overflow-hidden group`}
                                style={{ animationDelay: `${0.1 + index * 0.15}s` }}
                            >
                                {/* Gradient background */}
                                <div className={`absolute top-0 left-0 right-0 h-48 bg-gradient-to-b ${isAccent ? "from-accent/10" : "from-primary/10"} via-transparent to-transparent pointer-events-none`} />

                                <div className="relative z-10">
                                    {/* Header */}
                                    <div className="flex items-start justify-between mb-6">
                                        <div className={`w-12 h-12 rounded-xl ${isAccent ? "bg-accent/15" : "bg-primary/15"} border border-border flex items-center justify-center`}>
                                    ...
                                    <h3 className="font-heading font-bold text-xl text-card-foreground mb-1">{offer.title}</h3>
                                    <p className={`${isAccent ? "text-accent" : "text-primary"} text-sm font-medium mb-4`}>{offer.tagline}</p>

                                    {/* Description */}
                                    <p className="text-muted-foreground text-sm leading-relaxed mb-6">{offer.objectif}</p>
