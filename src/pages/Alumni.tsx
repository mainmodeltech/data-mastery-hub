 ...
 * AlumniPage.tsx
 *
 * Migration Supabase → backend Spring Boot :
 * - Alumni   : usePublishedAlumni()   → GET /api/v1/alumni       (public, camelCase)
 * - Projets  : usePublishedProjects() → GET /api/v1/projects      (public, camelCase)
 * - Détail   : usePublishedProjectDetail(id) → GET /api/v1/projects/:id
 *
 * Les types AlumniPerson et Project restent locaux à cette page
 * pour ne pas casser l'existant — ils sont mappés depuis les DTOs du backend.
 */
...
import {
  ...
  Briefcase, Quote, Trophy, Clock, Play, Pause, Loader2,
} from "lucide-react";
...
const staticAlumni: AlumniSummary[] = [
  { id: "a1", name: "Cedric Zagba",    currentTitle: "Analyste BI",         currentPosition: "Sonatel",            linkedinUrl: "#", photoUrl: null, cohort: "Promo Mars 2024", bootcampTitle: "Power BI",    published: true },
  { id: "a2", name: "Moussa Sow",      currentTitle: "Data Analyst",        currentPosition: "Wave Mobile Money",  linkedinUrl: "#", photoUrl: null, cohort: "Promo Jan 2024",  bootcampTitle: "Data Analyst", published: true },
  { id: "a3", name: "Aïssatou Fall",   currentTitle: "Contrôleur de Gestion", currentPosition: "Baobab Groupe",   linkedinUrl: "#", photoUrl: null, cohort: "Promo Nov 2023",  bootcampTitle: "Power BI",    published: true },
  { id: "a4", name: "Ibrahima Diallo", currentTitle: "SQL Developer",       currentPosition: "BICIS",              linkedinUrl: "#", photoUrl: null, cohort: "Promo Sept 2023", bootcampTitle: "Data Analyst", published: true },
  { id: "a5", name: "Mariama Ba",      currentTitle: "Business Analyst",    currentPosition: "Orange Senegal",    linkedinUrl: "#", photoUrl: null, cohort: "Promo Sept 2023", bootcampTitle: "Data Analyst", published: true },
  { id: "a6", name: "Cheikh Mbaye",    currentTitle: "Reporting Manager",   currentPosition: "CBAO",               linkedinUrl: "#", photoUrl: null, cohort: "Promo Juin 2023", bootcampTitle: "Power BI",    published: true },
];
...
function AlumniAvatar({ alumni, size = "sm" }: { alumni: Pick<AlumniSummary, "name" | "photoUrl">; size?: "sm" | "md" | "lg" }) {
  if (alumni.photoUrl) {
    ...
    return (
        <img
            src={alumni.photoUrl}
            alt={alumni.name}
            className={cn(sizes[size], "rounded-full object-cover flex-shrink-0")}
        />
    );
  }
  return <Avatar name={alumni.name} size={size} />;
}

// ─── Testimonial Card ─────────────────────────────────────────────────────────

function TestimonialCard({ t }: { t: Testimonial }) {
  const isBI       = t.bootcamp?.toLowerCase().includes("bi") || t.bootcamp?.toLowerCase().includes("power bi");
  ...
  return (
      <div className="flex-shrink-0 w-[340px] md:w-[400px] p-6 rounded-2xl border border-border bg-card flex flex-col">
        <div className="flex items-start justify-between mb-4">
          ...
          {t.bootcamp && (
              <span className={cn("text-xs px-2 py-0.5 rounded-full font-semibold flex items-center gap-1", colorClass)}>
        ...
        <div className="relative mb-5 flex-1">
          ...
          <p className="text-sm text-foreground leading-relaxed pl-4 italic">{t.content}</p>
        </div>
        {result && (
...
function ProjectCard({ project, onClick }: { project: ProjectSummary; onClick: () => void }) {
  const isPowerBI   = project.toolsTechnologies?.some(t => ["Power BI", "DAX", "Excel"].includes(t));
      ...
      <div className="rounded-2xl border border-border bg-card hover:shadow-card hover:-translate-y-1 transition-all duration-200 overflow-hidden flex flex-col">
          ...
          <div className={cn(
            ...
            {project.coverImageUrl ? (
                ...
                <>
                  ...
                  {isPowerBI
                      ? <BarChart3 className="h-16 w-16 text-accent/25" />
                      : <Database className="h-16 w-16 text-primary/25" />
                  }
                </>
            )}
            {/* Badge cohorte */}
        ...
        <div className="p-5 flex flex-col flex-1">
          <button onClick={onClick} className="text-left mb-2">
            <h3 className={cn("font-heading font-bold text-base text-foreground leading-tight", `hover:${accentColor} transition-colors`)}>
              {project.title}
            </h3>
          </button>
                ...
                <div className="flex flex-col gap-2">
                  {project.members.map(m => (
                      <div key={m.id} className="flex items-center gap-2">
                        <AlumniAvatar alumni={m.alumni} size="sm" />
                        <div className="flex-1 min-w-0">
                          <div className="text-xs font-semibold text-foreground leading-tight">{m.alumni.name}</div>
                          {(m.alumni.currentTitle || m.alumni.currentPosition) && (
                              <div className="text-xs text-muted-foreground truncate">
                                {[m.alumni.currentTitle, m.alumni.currentPosition].filter(Boolean).join(" · ")}
                              </div>
                          )}
                        </div>
                        {m.alumni.linkedinUrl && (
                            <a href={m.alumni.linkedinUrl} target="_blank" rel="noopener noreferrer"
...
function ProjectDetailModal({
  ...
  const { data: project, isLoading } = usePublishedProjectDetail(projectId);
  const [photoIdx, setPhotoIdx]      = useState(0);

  // Reset photo index quand le projet change
  useEffect(() => { setPhotoIdx(0); }, [projectId]);

  if (!projectId) return null;

  const screenshots = project?.screenshots ?? [];

  const prevPhoto = () => setPhotoIdx(i => (i === 0 ? screenshots.length - 1 : i - 1));
  const nextPhoto = () => setPhotoIdx(i => (i === screenshots.length - 1 ? 0 : i + 1));
        ...
        <div className="relative w-full max-w-2xl bg-card rounded-3xl border border-border shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
          ...
          <div className="flex items-start justify-between p-6 border-b border-border flex-shrink-0">
            ...
            <button onClick={onClose}
                    className="w-8 h-8 rounded-full bg-secondary flex items-center justify-center hover:bg-muted transition-colors flex-shrink-0">
              <X className="h-4 w-4" />
            </button>
          </div>
          ...
          <div className="overflow-y-auto flex-1 p-6 space-y-5">
            {isLoading ? (
                ...
                <>
                  ...
                  {/* Outils */}
                  {(project.toolsTechnologies?.length ?? 0) > 0 && (
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {(project.toolsTechnologies ?? []).map(t => (
                              <span key={t} className={cn("text-xs px-3 py-1 rounded-lg border font-semibold", getToolClass(t))}>
                        {t}
                      </span>
                          ))}
                        </div>
                  ...
                  {project.members.length > 0 && (
                      <div>
                        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3 flex items-center gap-1.5">
                          <Users className="h-3.5 w-3.5" /> L'équipe
                        </p>
                        <div className="grid sm:grid-cols-2 gap-3">
                          {project.members.map(m => (
                              <div key={m.id} className="flex items-center gap-3 p-3 rounded-xl bg-secondary/40 border border-border">
                                <AlumniAvatar alumni={m.alumni} size="sm" />
                                <div className="flex-1 min-w-0">
                                  <p className="text-sm font-semibold text-foreground leading-tight">{m.alumni.name}</p>
                                  {m.alumni.currentTitle && (
                                      <p className="text-xs text-muted-foreground truncate">{m.alumni.currentTitle}</p>
                                  )}
                                </div>
                                {m.alumni.linkedinUrl && (
                                    <a href={m.alumni.linkedinUrl} target="_blank" rel="noopener noreferrer"
                                       className="w-7 h-7 rounded-lg border border-border flex items-center justify-center text-muted-foreground hover:text-primary transition-colors flex-shrink-0">
                  ...
                  {screenshots.length > 0 && (
                      <div>
                        ...
                        <div className="relative rounded-xl overflow-hidden border border-border">
                          <img
                              src={screenshots[photoIdx].photoUrl}
                              alt={screenshots[photoIdx].caption ?? ""}
                              className="w-full h-56 object-cover"
                          />
                          {screenshots[photoIdx].caption && (
                              <div className="absolute bottom-0 left-0 right-0 bg-foreground/60 px-3 py-2">
                                <p className="text-xs text-background">{screenshots[photoIdx].caption}</p>
                              </div>
                          )}
                ...
                <p className="text-muted-foreground text-sm text-center py-8">Projet introuvable.</p>
            )}
          </div>
        </div>
      </div>
...
function AlumniCard({ person, delay }: { person: AlumniSummary; delay: number }) {
  return (
      <div
...
const AlumniPage = () => {
  ...
  // ── Projets → backend Spring Boot (plus de Supabase) ───────────────────────
  const { data: dbProjects, isLoading: loadingProjects } = usePublishedProjects();
  const projects = dbProjects ?? [];

  // ── Alumni → backend Spring Boot (plus de Supabase) ────────────────────────
  const { data: dbAlumni, isLoading: loadingAlumni } = usePublishedAlumni();
  ...
  return (
      <Layout>
                ...
                <div className="mt-12 max-w-2xl mx-auto rounded-2xl border border-primary/25 bg-primary/5 p-6 flex flex-col sm:flex-row items-center gap-4">
                  ...
                  <div className="flex-1 text-center sm:text-left">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Nos alumni peuvent réaliser ce type de projet pour vous. Contactez-nous pour un devis gratuit.
                    </p>
                  </div>
        ...
        {/* ── ANNUAIRE ALUMNI → backend Spring Boot ────────────────────────────── */}
                  ...
                  {alumni.length > 12 && (
                      <div className="text-center">
                        <Button variant="outline" onClick={() => setShowAllAlumni(!showAllAlumni)}>
                          {showAllAlumni
                              ? "Voir moins"
                              : `Voir les ${alumni.length - 12} autres alumni`}
                  ...
                  <div className="flex flex-wrap gap-3 mt-5 text-xs text-background/40">
                    {["Financement possible", "Certification incluse", "Accès réseau alumni"].map(t => (
                        <div key={t} className="flex items-center gap-1">
                          <CheckCircle className="h-3 w-3 text-accent/60" />{t}
                        </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="rounded-3xl border-2 border-primary/25 bg-primary/5 p-8 lg:p-10 relative overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-muted-foreground text-sm leading-relaxed mb-6">
                    Accédez à notre vivier de talents certifiés ou confiez vos projets data à nos équipes.
                  </p>
                  <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-3 mt-5 text-xs text-muted-foreground">
                    {["Profils pré-sélectionnés", "Devis sous 24h", "Sans engagement"].map(t => (
                        <div key={t} className="flex items-center gap-1">
                          <CheckCircle className="h-3 w-3 text-primary/60" />{t}
                        </div>
                    ))}
                  </div>
                </div>
